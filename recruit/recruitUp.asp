...
<script type="text/javascript">
	function chkJoinForm(){
		...
		if (!$("#selPartnerD1_1 option:selected").val()){
			alert("대분류를 선택해 주세요.");
			return;
		}
		
		if ($("#selPartnerD1_1 option:selected").val() == "3"){
			if (!$("#frmJoin").find("#recommend_num").val()){
				alert("추천인 사원번호을 입력해주세요.");
				$("#frmJoin").find("#recommend_num").focus();
				return;
			} else if ($("#frmJoin").find("#recommend_num").val().length < 8){
				alert("정확한 사원번호(8자리)를 입력해주세요.");
				$("#frmJoin").find("#recommend_num").focus();
				return;
			}   else if (!$("#frmJoin").find("#recommend_name").val()){
				alert("추천인 성함을 입력해주세요.");
				$("#frmJoin").find("#recommend_name").focus();
				return;
			}  else if (!$("#frmJoin").find("#recommend_center").val()){
				alert("지원센터명을 입력해주세요.");
				$("#frmJoin").find("#recommend_center").focus();
				return;
			} 
		}else{
			$("#recommend_num").val("");
			$("#recommend_name").val("");
			$("#recommend_center").val("");
		}
    ...
    $('#frmJoin').ajaxForm({
            ...
            success: function(response,status){
				...
				if (response=="SUCC") {
					...
					var RecrSeq  ="<%=RecrSeq%>";
					var PartnerDepth1 = $("#PartnerDepth1").val();
					var PartnerDepth2 = $("#PartnerDepth2").val();
					var recommend_num = $("#recommend_num").val();
					var recommend_name		= $("#recommend_name").val();
					var recommend_center	= $("#recommend_center").val();
					
					var param = "RecrSeq=" + RecrSeq + "&PartnerDepth1=" + PartnerDepth1 + "&PartnerDepth2=" + PartnerDepth2 + "&recommend_num=" + recommend_num + "&recommend_name=" + recommend_name + "&recommend_center=" + recommend_center;


					openLayer("/recruit/apply_proc.asp",param+"&NextAction=APPLY", "w668");
					<% End If %>
	...
	$(function() {
		...
		$("#recommend_num").keypress(function(e){ return OnlyNumber(e,false); });
		$("#recommend_num").keyup(function(){ $(this).val( FillNumerics($(this).val())); });
	});
		...
		<div id="container">
			...
			<div class="inner"><div class="inner2">
				<div class="popCont easySignUp">
					<% If PageFlag = "JOIN" Then %>
					<h2><img src="/images/pages/tit_signup_n.png" alt="간편회원가입" /></h2>
					<p class="note">
					...
					<% Else %>
					<h2><img src="/images/pages/tit_easyRecruit_n.jpg" alt="간편입사지원" /></h2>
					<p class="note">
						...
						<img src="/images/pages/txt_easyRecruite_04_n.gif" alt="지원서 저장시 자동으로 히ㅗ원가입됩니다.(ID : 휴대폰번호)" /><br />
						<img src="/images/pages/txt_easyRecruite_05_n.gif" alt="지원서 저장시 자동 로그인 됩니다." />
					</p>
					<% End If %>
					
					<form id="frmJoin" name="frmJoin" method="post" action="https://www.mpcjob.co.kr/membership/easySignUp_proc.asp">
					...
					<div class="board_write">
						<table>
							...
							<tbody>
								<tr>
									<th><label for="UsrName"><span class="ess">* </span>지원경로</label></th>
									<td colspan="3">
										<select id="selPartnerD1_1" name="selPartnerD1_1" class="w100">
											<option value=''>대분류</option>
											<option value='5'>온라인광고</option>
											<option value='2'>MPCJOB</option>
											<option value='3'>직원추천</option>
										</select>
										<span id="divPartner_1"></span>
									</td>
								</tr>
								<tr id="recommend_info" style="display:none">
									<th><label for="UsrName"><span class="ess">* </span>추천정보</label></th>
									<td colspan="3">
										<span>
											추천인 사원번호 <input type="text" id="recommend_num" name="recommend_num" maxlength="8" class="w110" style="ime-mode:disabled;">
											추천인 성함	  <input type="text" id="recommend_name" name="recommend_name" class="w110">
											지원센터명     <input type="text" id="recommend_center" name="recommend_center" class="w110">
										</span>
									</td>
								</tr>
								<tr>
					...
					<div class="inner_txt">
						<h4>제1조 (목적)</h4>
						<p>본 약관은 (주)엠피씨플러스에서 운영하는 취업정보 서비스에 대한 회원제도에 대한 제반사항을 규정하는데 목적이 있습니다.</p>
						<h4>제2조 (정의)</h4>
					...
					<div class="inner_txt">
						...
						(주)엠피씨플러스은 개인정보보호법, 정보통신망 이용 촉진 및 정보보호 등에 관한 법률 등 관련 법령상의 개인정보보호 규정을 준수하며, 지원자의 개인정보보호에 최선을 다하고 있습니다.<br />
						엠피씨플러스 사이트 내에서 주민등록번호를 취득하거나 이용하지 않습니다.<br />
						엠피씨플러스의 기가입자의 주민등록번호는 모두 폐기합니다.<br />
						엠피씨플러스은 개인식별을 위하여 이름/생년월일/휴대폰전화번호를 주민등록번호의 대체수단으로 이용합니다.<br />
						엠피씨플러스 사이트는 본인의 동의 없이 개인정보를 수집 및 활용하거나 제3자에게 제공하지 않습니다.<br />
						<h4>제1조 개인정보의 수집 및 이용목적</h4>
							(주)엠피씨플러스에서 운영하는 고객센터의 채용 지원과 자격요건의 확인 및 원활한 채용 진행, 진행단계별 결과, 채용관련 정보를 안내하기 위함.
