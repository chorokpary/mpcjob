	...
	<script type="text/javascript">
		function doApply(){
			...
			if ($("#selPartnerD1_1 option:selected").val() == "3"){
				if (!$("#recommend_num").val()){
					alert("추천인 사원번호을 입력해주세요.");
					$("#recommend_num").focus();
					return;
				} else if ($("#recommend_num").val().length < 8){
					alert("정확한 사원번호(8자리)를 입력해주세요.");
					$("#recommend_num").focus();
					return;
				}   else if (!$("#recommend_name").val()){
					alert("추천인 성함을 입력해주세요.");
					$("#recommend_name").focus();
					return;
				}  else if (!$("#recommend_center").val()){
					alert("지원센터명을 입력해주세요.");
					$("#recommend_center").focus();
					return;
				} 
			}else{
				$("#recommend_num").val("");
			...
			<% Else %>
				<% If Session("USeq") = "" Then %>
				//로그인 요청창 출력
					location.href = "./recruitUp_t.asp?RecrSeq=<%=Seq%>"
				<% Else %>
				//입사지원
				//입사지원시 분류 및 추천인 정보 전달 요망 
				var RecrSeq  ="<%=Seq%>";
				...
				openLayer("/recruit/apply_proc_t.asp",param, "w668");
				<% End If %>
			<% End If%>
		}
	...
	<div id="wrap" class="recruitView">
		...
		<div id="container">
			<div class="contents">
					...
					<table>
						...
						<tbody>
							<tr>
								...
								<td><%If ChargePhone <> "" Then Response.write oSeed.Decrypt(ChargePhone) End If %></td>
							</tr>
						</tbody>
					</table>
				...
				<div class="btn_apply">
					<a href="javascript:;" id="btnQNA"><img src="/images/pages/btn_inquiry.jpg" alt="문의하기" /></a>
				</div>
			</div>
