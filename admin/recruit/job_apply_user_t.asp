...
<%
	...
	Sub Main()
		...
		If Session("ALevel") <> "1" AND Session("ALevel") <> "2" Then	' 최종관리자가 아닐경우 접근 권한 체크_최종관리자2 추가 
			Call chkAuth()
		End If
	...
	Sub getList()
		...
		Else
			...
			' CntMemo(17), Status(18), CntAll(19), CntIng(20), CntFin(21), UsrType(22), Mobile(23), IsBlack(24), CancelDate(25), IsOut(26), CntStat(27), IsSearch(28)
			kFields 	= Array("TotalCnt","AppSeq","UsrSeq","PartnerDepth1","PartnerDepth2","RegDate","UsrName","Gender","Birthday","AddrSido","AddrGugun" _
							,"Attach1_Path","Attach1_FName","Attach2_Path","Attach2_FName","Attach3_Path","Attach3_FName","CntMemo","Status","CntAll","CntIng","CntFin","UsrType","Mobile","IsBlack","CancelDate","IsOut","CntStat","IsSearch")
	...
	Function Fn_GetAge(val)
		...
		check_month = now_month - user_ymd(1) 
		check_day = now_day - user_ymd(2)
		
		If check_month >= 0 Then '생일이 지나지 않은
			If check_day >= 0 Then 
				age = age -1
			End If 					
		End If 

		Fn_GetAge= age & check_month &  check_day
	End Function
						...
						<tbody>
							...
							<% 		
									...
									For i = 0 To arrDataNum 
										...
										black_chk = "N" 
										If arrData(23,i) <> "" Then

											'Sql_black_chk = " SELECT UsrSeq FROM TBL_USER_201511_before_black Where UsrID = '"&oSeed.Decrypt(arrData(23,i))&"' "
											Sql_black_chk = " SELECT UsrSeq FROM TBL_USER Where UsrID = '"&oSeed.Decrypt(arrData(23,i))&"' "
											With objCmd
												.ActiveConnection = objDbCon
												.CommandType = 1 ' adCmdText
												.CommandText = Sql_black_chk
												Set Rs_black = .Execute
											End With

											if Rs_black.eof or Rs_black.bof then    '실행결과가 없을 경우(예외처리)
												black_chk = "N"
											Else
												black_chk = "Y"
											End If 
										...
										if isNull(arrData(27,i))  then
										   CntStat = 0 
										else 
										   CntStat = arrData(27,i)
										end if 

										If arrData(24,i) Or UCase(black_chk) = "Y" Then
											strAppCStyle = strAppCStyle & " blackList"
										ElseIf CStr(Trim(arrData(28,i))) = "True" Then 
											strAppCStyle = "searchList"
										Else
											If CInt(CntStat) > 0 then
												strAppCStyle = "statList"
											Else
												strAppCStyle = ""
											End If
										End If
							...
							<tr <%=strAppCStyle%>>
								...
								<td><% If IsNumeric(Left(arrData(8,i),4)) Then %><%=Fn_GetAge(arrData(8,i))%><% Else %> - <% End If %><%=arrData(8,i)%></td>
								<td><%=arrData(9,i)%>&nbsp;<%=arrData(10,i)%></td>
								<td><%If arrData(23,i) <> "" Then response.write oSeed.Decrypt(arrData(23,i)) End If%></td>
								<td><%=arrData(18,i)%><%=strAppCMsg%></td>
								<td>
								<%
								if Cint(CntStat) > 0 then
								%>
									<a href="javascript:goUserApply('<%=arrData(2,i)%>');"><font color="#ff0000">총<%=arrData(19,i)%>건</font></a>
								<%else%>
									<a href="javascript:goUserApply('<%=arrData(2,i)%>');">총<%=arrData(19,i)%>건</a> 
								<%end if%> 
								(마감:<%=arrData(21,i)%>, 진행:<%=arrData(20,i)%>)</td>
