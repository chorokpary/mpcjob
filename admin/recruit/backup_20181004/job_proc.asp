...
<%
	...
	Sub setProc()
		...
		With objCmd
			...
			.Parameters("@ChargePhone").Value = ChargePhone
		...
		Else
			If Result = 0 Then
				...
				If Flag = "ADD" OR Flag = "REF" Then
					strScript = "window.location.href = ""job_skin_list.asp""; "
				ElseIf Flag = "MOD" Then
					strScript = "$(""#frmPage"").attr({action:""job_skin_" & Fn_SetDefault(IsView,True,"list","elist") & ".asp"", method:""post""}).submit();"
				ElseIf Flag = "NO_RECOM" Then
					strScript = "window.location.href = ""job_recom_list.asp?sViewFlag=" & sViewFlag & """; "
				Else
					strScript = "window.location.href = ""job_skin_elist.asp""; "
				End If
			Else
