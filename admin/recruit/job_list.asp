...
<%
	...
	Sub getList()
		...
		Else
			...
			' PrjName(7), AdmName(8), AppCnt(9), IsView(10), Times(11)
			
			kFields 	= Array("TotalCnt","RecrSeq","Title","IsMain","IsTop","AppBDate","AppEDate","PrjName","AdmName","AppCnt","IsVIew","Times")
	...
	<script type="text/javascript">
		...
		function chkXls(){
			if (cntChecked("#frmList")==0){
				...
				if(confirm("선택한 채용공고를 엑셀로 다운 받으시겠습니까?")){
					$("#Seq").val(getCheckedVal("Seq"));
					$("#frmPage").attr({action:"job_apply_user_xls_new.asp", method:"post", target:"_self"}).submit();
					$('#frmPage').attr('action', '<%=request.ServerVariables("PATH_INFO")%>');
			...
			$("#sEDate").datepicker({
				...
				, current: $("#EDate").val()
					...
					<div class="btnArea">
						...
						<div class="fl_r">
							<span class="btns btn_b"><a href="#" id="btnXls">지원자리스트 저장</a>
							</span><span class="btns btn_b"><a href="#" id="btnApp">지원자현황</a>
					...
					<div class="tblArea">
						<table>
							...
							<tbody>
								<% If Not kData Then %>
								<tr>
									<td colspan="8">등록된 채용공고가 없습니다.</td>
								</tr>
								<% Else %>
								<% 		
										' TotalCnt(0), RecrSeq(1), Title(2), IsMain(3), IsTop(4), AppBDate(5), AppEDate(6)
