...
<%
	...
	Sub getList2()
		...
		Else 
		End If 
		If sBirth <> "" Then 	Sql_Where = Sql_Where & " AND (LEFT(D.Birthday,4) >= '"&sBirth&"' AND LEFT(D.Birthday,4) <= '"&sBirth2&"')"
		If sSearchHistory <> "" Then
...
'			.Parameters.Refresh
		...
		Sql = 	" SELECT T2.TotalCnt, T2.UsrSeq, T2.UsrID, T2.UsrName, T2.Gender, T2.Birthday, T2.AddrSido, T2.AddrGugun, T2.Mobile, T2.HopePart, T2.HopePart2, T2.RegDate, T2.ModDate, T2.IsBlack, T2.Status, T2.PartnerDepth1, T2.PartnerDepth2 , T2.PrjName , T2.EzCareer, T2.CntEzCareer, T2.RecrPart, T2.RecrPart2, T2.UsrType, T2.IsSearch " &_
				...
				" FROM ( " &_
				" 	SELECT TOP "& listSize &" T.TotalCnt, T.UsrSeq, T.UsrID, T.UsrName, T.Gender, T.Birthday, T.AddrSido, T.AddrGugun, T.Mobile, T.HopePart, T.HopePart2, T.RegDate, T.ModDate, T.IsBlack, T.Status, T.PartnerDepth1, T.PartnerDepth2 , T.PrjName , T.EzCareer, T.CntEzCareer, T.RecrPart, T.RecrPart2, T.UsrType, T.IsSearch " &_
				" 	FROM (" &_
		...
		Else
			...
			kFields 	= Array("TotalCnt","UsrSeq","UsrID","UsrName","Gender","Birthday","AddrSido","AddrGugun","Mobile","HopePart","RegDate","ModDate","IsBlack","CntMemo","Status","PartnerDepth1","PartnerDepth2","PrjName","EzCareer","CntEzCareer","HopePart2","RecrPart","RecrPart2","UsrType","IsSearch")
