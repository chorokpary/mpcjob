...
<%
	...
	Dim sGender, sHope, sHope2, sStatus, sSido, sGugun, sBDate, sEDate, sBlack
	...
	Sub getList2()
		...
		If sBlack <> "" Then 	Sql_Where = Sql_Where & " AND D.IsBlack = ? "
		If sSearchHistory <> "" Then
...
'			.Parameters.Refresh
		...
		Sql = 	" SELECT T2.TotalCnt, T2.UsrSeq, T2.UsrID, T2.UsrName, T2.Gender, T2.Birthday, T2.AddrSido, T2.AddrGugun, T2.Mobile, T2.HopePart, T2.HopePart2, T2.RegDate, T2.ModDate, T2.IsBlack, T2.Status, T2.PartnerDepth1, T2.PartnerDepth2 , T2.PrjName , T2.EzCareer, T2.CntEzCareer, T2.RecrPart, T2.RecrPart2, T2.UsrType " &_
				" 	, ISNULL((SELECT COUNT(*) AS CntMemo FROM TBL_USER_MEMO WITH (nolock) WHERE UsrSeq = T2.UsrSeq),0) AS CntMemo" &_ 	
				" FROM ( " &_
				" 	SELECT TOP "& listSize &" T.TotalCnt, T.UsrSeq, T.UsrID, T.UsrName, T.Gender, T.Birthday, T.AddrSido, T.AddrGugun, T.Mobile, T.HopePart, T.HopePart2, T.RegDate, T.ModDate, T.IsBlack, T.Status, T.PartnerDepth1, T.PartnerDepth2 , T.PrjName , T.EzCareer, T.CntEzCareer, T.RecrPart, T.RecrPart2, T.UsrType " &_
				" 	FROM (" &_
				" 		SELECT TOP " & (Page * listRecord) & " 0 AS TotalCnt, D.UsrSeq " &_
				" 		, D.UsrID, D.UsrName, D.Gender, D.Birthday, D.AddrSido, D.AddrGugun, D.Mobile, D.HopePart, D.HopePart2, D.RegDate, D.ModDate, D.IsBlack, Isnull(D.UsrType,0) as UsrType  " &_
				" 		, CC.CdVal AS Status, AI.PartnerDepth1, AI.PartnerDepth2 , P.PrjName " &_
				" 		, EC.EzCareer, EC.CntEzCareer " &_
				" 		, R.RecrPart, R.RecrPart2 " &_
		...
		Else
			...
			' CntMemo(13), Status(14), PartnerDepth1(15), PartnerDepth2(16), PrjName(17), EzCareer(18), CntEzCareer(19), HopePart2(20), RecrPart(21), RecrPart2(22)
			kFields 	= Array("TotalCnt","UsrSeq","UsrID","UsrName","Gender","Birthday","AddrSido","AddrGugun","Mobile","HopePart","RegDate","ModDate","IsBlack","CntMemo","Status","PartnerDepth1","PartnerDepth2","PrjName","EzCareer","CntEzCareer","HopePart2","RecrPart","RecrPart2","UsrType")
	...
	Sub getList()
		...
		Else
			...
			' AddrGugun(7), Mobile(8), HopePart(9), RegDate(10), ModDate(11), IsBlack(12)
			' CntMemo(13), Status(14), PartnerDepth1(15), PartnerDepth2(16), PrjName(17), EzCareer(18), CntEzCareer(19), HopePart2(20), RecrPart(21), RecrPart2(22)
			kFields 	= Array("TotalCnt","UsrSeq","UsrID","UsrName","Gender","Birthday","AddrSido","AddrGugun","Mobile","HopePart","RegDate","ModDate","IsBlack","CntMemo","Status","PartnerDepth1","PartnerDepth2","PrjName","EzCareer","CntEzCareer","HopePart2","RecrPart","RecrPart2")
			arrData 	= Rs.GetRows(,,kFields)
			arrDataNum 	= UBound(arrData, 2)
			kData 		= True
					...
					<div class="tblArea">
						<table>
							...
							<tbody>
								...
								%>
								<tr<%=Fn_SetDefault(arrData(12,i),True," class=""blackList""","")%> >
