...
<%
	...
	Sub Main()
		...
		listPage 	= 10
		listRecord 	= FN_ReqF("ListSize","20")	'목록갯수
		Page 		= FN_ReqF("Page","1")
...
'			.Parameters.Refresh
		...
		Sql = 	" SELECT T2.TotalCnt, T2.UsrSeq, T2.UsrID, T2.UsrName, T2.Gender, T2.Birthday, T2.AddrSido, T2.AddrGugun, T2.Mobile, T2.HopePart, T2.HopePart2, T2.RegDate, T2.ModDate, T2.IsBlack, T2.Status, T2.PartnerDepth1, T2.PartnerDepth2 , T2.PrjName , T2.EzCareer, T2.CntEzCareer, T2.RecrPart, T2.RecrPart2 " &_
				" 	, ISNULL((SELECT COUNT(*) AS CntMemo FROM TBL_USER_MEMO WITH (nolock) WHERE UsrSeq = T2.UsrSeq),0) AS CntMemo" &_ 	
				" FROM ( " &_
				" 	SELECT TOP "& listSize &" T.TotalCnt, T.UsrSeq, T.UsrID, T.UsrName, T.Gender, T.Birthday, T.AddrSido, T.AddrGugun, T.Mobile, T.HopePart, T.HopePart2, T.RegDate, T.ModDate, T.IsBlack, T.Status, T.PartnerDepth1, T.PartnerDepth2 , T.PrjName , T.EzCareer, T.CntEzCareer, T.RecrPart, T.RecrPart2 " &_
				" 	FROM (" &_
				" 		SELECT TOP " & (Page * listRecord) & " 0 AS TotalCnt, D.UsrSeq " &_
				" 		, D.UsrID, D.UsrName, D.Gender, D.Birthday, D.AddrSido, D.AddrGugun, D.Mobile, D.HopePart, D.HopePart2, D.RegDate, D.ModDate, D.IsBlack " &_
