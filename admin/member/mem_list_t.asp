...
<%
	...
	Dim PartnerDepth1, PartnerDepth2, selPartnerD1_1
	...
	Sub Main()
		...
		selPartnerD1_1 	= FN_Req("selPartnerD1_1","")
	...
	Sub getList2()
		...
		If PartnerDepth1 <> "" Then 	Sql_Where = Sql_Where & " AND AI.PartnerDepth1 = ? "
		If PartnerDepth2 <> "" Then 	Sql_Where = Sql_Where & " AND AI.PartnerDepth2 = ? "
		If sBlack = "1" Then 	
		...
		If sSearchHistory <> "" Then
			For n = 0 To UBound(arrSearch)
				Select Case Trim(arrSearchData(n)(0))
					...
					Case "MOBILE" : Sql_Where = Sql_Where & " AND D.Mobile LIKE '%' + ? + '%' "
...
'			.Parameters.Refresh
			...
			If PartnerDepth1 <> "" Then	.Parameters.Append .CreateParameter("PartnerDepth1", adVarWChar, adParamInput, 30, PartnerDepth1)
			If PartnerDepth2 <> "" Then	.Parameters.Append .CreateParameter("PartnerDepth2", adVarWChar, adParamInput, 30, PartnerDepth2)
			...
			If sSearchHistory <> "" Then
				For n = 0 To UBound(arrSearch)
					Select Case Trim(arrSearchData(n)(0))
						...
						Case "MOBILE"
							.Parameters.Append .CreateParameter("sWord", adVarWChar, adParamInput, 200, Replace(Replace(Trim(oSeed.Encrypt(arrSearchData(n)(1))),"[","[[]"),"-",""))
						Case "ADDR" : 
...
<head>
	...
	<script type="text/javascript">
		...
		$(function() {
		    ...
		    $("#selPartnerD1_1").change(getPartner_search);
	...
	<div id="wrap">
		...
		<div class="contents2">
			...
			<div class="right">
				...
				<div id="contArea">
					<h2>회원관리 ( 오늘 가입한 신규 회원수는 [ <%=StrCntToday%> ]명 입니다. )[<%=Session("ALevel")%>]</h2>
					<form name="frmPage" id="frmPage" method="post" action="mem_list_t.asp">
					<input type="hidden" id="Page" name="Page">
					<input type="hidden" id="SortKey" name="SortKey" value="<%=SortKey%>">
					<input type="hidden" id="SortDir" name="SortDir" value="<%=SortDir%>">

					<input type="hidden" id="PartnerDepth1" name="PartnerDepth1" value="<%=PartnerDepth1%>">
					<input type="hidden" id="PartnerDepth2" name="PartnerDepth2" value="<%=PartnerDepth2%>">
					...
					<div class="searchArea">
						...
						<div class="search_detail pt10">
							<div class="fl_l">
								...
								<span class="group">
									<span class="tit">지원경로</span> 
									<%
									Dim PartnerSql_search : PartnerSql_search = "Execute uspPartner_Info @Flag = 'CDD1'" 
									response.write " ": Call SB_MakeHTMLSelect(objDBCon, PartnerSql_search, "selPartnerD1_1", "대분류", "", selPartnerD1_1, "", " class=""w100""", False)
									%>
									<span id="divPartner_1">
									<%
									Dim PartnerSql_search_sub : PartnerSql_search_sub = "Execute uspPartner_Info @Flag = 'CDD2', @Seq='" & selPartnerD1_1 & "'" 
									response.write " ": Call SB_MakeHTMLSelect(objDBCon, PartnerSql_search_sub, "selPartnerD2_1", "중분류", "", PartnerDepth2, "", " class=""w110"" ", False)
									%>
									</span>
								</span>
						...
						<table>
							...
							<tbody>
								...
								<tr class="<%=class_txt%>" >
									...
									<td><%=vNum%></td>
									<td><a href="mem_form.asp?Seq=<%=arrData(1,i)%>&Filter=<%=strFilter%>"><%=arrData(3,i)%></a>/<%=arrData(1,i)%></td>
									<td>
										<%
									...
									<td><%=oSeed.Decrypt(arrData(8,i))%></td>
									<td>
										<%=arrData(21,i)%>
