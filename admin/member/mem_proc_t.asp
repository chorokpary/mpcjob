...
<%
	...
	Sub Main()
		...
		If Flag = "DEL" Or Flag = "BLK" Or Flag = "SEARCH" Or Flag = "RECR" Or Flag = "PWD_C" Or Flag = "PWD_S" _
			Or Flag = "EDU" Or Flag = "CAREER" Or Flag = "FAM" Or Flag = "MEMO" Or Flag = "QUES" Or Flag = "AGREE" Or Flag = "PWD_R"  Then
			' DEL : 관리자삭제 / BLK : 블랙리스트 처리 / SEARCH : 서칭거부리스트 처리 / RECR : 채용공고 지원 / PWD_C : 비밀번호변경 / PWD_S : 임시비밀번호 전송
			' EDU : 학력정보 / CAREER : 경력정보 / FAM : 가족사항 / MEMO : 관리자 메모 / QUES : 사전질문 답변 / PWD_R : 비밀번호 횟수 초기화
			Call getParamReq()
		Else
	...
	Sub getParamReq()
		...
		If Flag = "PWD_C" Or Flag = "PWD_S" Or Flag = "PWD_R" Then
			strParamFlag = "@Seq"
	...
	Sub setProc()
		...
		If Flag = "PWD_S" Then
			...
			tmpPasswd = LEFT(tmpPasswd,4)
			tmpPasswd = UCase(tmpPasswd)
			tmpPasswd = Left(tmpPasswd,2) & random_tmp_str_1() & random_tmp_str_2() & Right(tmpPasswd,2)
	...
	Sub setResult()
		...
		Else
			If Result = 0 Then
				...
				ElseIf Flag = "MOD" Then
					strScript = "window.location.href = ""http://www.mpcjob.co.kr/admin/member/mem_form.asp?Seq=" & Seq & "&Filter=" & Filter & "&sFlag=" & sFlag & "&sView=" & sView & """; "
				ElseIf Flag = "MEMO" Then
