...
<head>
	...
	<script type="text/javascript">
		...
		function goDownList(seq){
			var win = 	openWin("adm_down_list.asp?AdmSeq="+ seq,"",0,0,1020,700,0);
			win.focus();
		}
					...
					<div class="tblArea">
						...
						<table>
							...
							<tbody>
								<% If Not kData Then %>
								<tr>
									<td colspan="10">등록된 관리자가 없습니다.</td>
								</tr>
								<% Else %>
								<% 		
										' TotalCnt(0), AdmSeq(1), AdmID(2), AdmName(3), AdmLevel(4), AdmEmail(5), VisitDate(6), AdmMobile(7), AdmTel(8)
								...
								<tr>
									...
									<td><%=arrData(6,i)%></td>
									<td><%If arrData(7,i) <> "" Then Response.write oSeed.Decrypt(arrData(7,i)) End If%></td>
									<td><%If arrData(8,i) <> "" Then Response.write oSeed.Decrypt(arrData(8,i)) End If%></td>
									<td><a href="javascript:goDownList('<%=arrData(1,i)%>')">[ LIST ]</a></td>
								</tr>
								<%			vNum = vNum - 1
										Next 
								%>
