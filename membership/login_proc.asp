...
<% 
	...
	Sub getParam()
		...
		If strID = "" Or strPWD = "" Then 		Call SB_ReturnErr("접속 정보가 올바르지 않습니다.","http://www.mpcjob.co.kr/membership/login.asp")
		...
		With objCmd
			...
			If Result = "SUCC" Then
				...
				If PageFlag = "FRONT" Then
					...
					Sql = " UPDATE TBL_USER SET MissCnt = 0 Where UsrSeq = '"&Rs("UsrSeq")&"' "
					With objCmd
						.ActiveConnection = objDbCon
					...
					Response.Write "callback({'result':'"&Result&"|S'})"
			...
			ElseIf Result = "LOCK" Then
				...
				Response.Write "callback({'result':'"&Result&"|L'})"
				Response.End
			...
			'	Call SB_ReturnErr("접속 정보가 올바르지 않습니다.","SELF")
				...
				If PageFlag = "FRONT" Then
					...
					Sql = " SELECT MissCnt FROM TBL_USER Where UsrID = '"&oSeed.Encrypt(strID)&"' and IsOut ='0' "
					With objCmd
						.ActiveConnection = objDbCon
						.CommandType = 1 ' adCmdText
						.CommandText =Sql
					...
					Response.Write "callback({'result':'"&Result&"|"&Rs(0)&"'})"
					Response.End
				ElseIf PageFlag = "ADM" Then
					Call SB_ReturnErr("접속 정보가 올바르지 않습니다.","http://www.mpcjob.co.kr/membership/login.asp")
				End If
			Else
